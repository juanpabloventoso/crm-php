...
class EditableGrid {
	...
	private function getRowXML($DOMDocument, $row, $customRowAttributes, $encodeCustomAttributes)
	{
		...
		$rowNode->setAttribute('id', $this->_getRowField($row, 'id' . ucfirst($this->tabla)));
	...
	{
		$data = array("id" => $this->_getRowField($row, 'id' . ucfirst($this->tabla)), "values" => array());
